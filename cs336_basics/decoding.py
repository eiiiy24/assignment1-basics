...
def decode(
    ...
    token_ids = torch.tensor([tokenizer.encode(prompt)], device=next(model.parameters()).device, dtype=torch.long)  # shape (1, seq)
    ...
    while i < max_tokens_num:
        ...
        token_ids = torch.cat([token_ids, torch.tensor([[next_token]], device=next(model.parameters()).device, dtype=torch.long)], dim=1) # 这里怎么办，应该加进去
        if len(token_ids[0]) > model.context_length:
            token_ids = token_ids[:, -model.context_length:]
